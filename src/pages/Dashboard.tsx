...
export default function Dashboard() {
  ...
  const getSupportGiven = () =>
    supports.filter((s) => myEventIds.includes(s.supporter_event_id));
  const getTotalReceived = (eventId: string) =>
    getSupportersForEvent(eventId).reduce(
      (sum, s) => sum + Number(s.amount),
      0,
    );

  const totalSupportReceived = events.reduce(
    (sum, ev) => sum + getTotalReceived(ev.id),
    0,
  );
  const supportGiven = getSupportGiven();
  const totalSupportGiven = supportGiven.reduce(
    (sum, s) => sum + Number(s.amount),
    0,
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {events.length > 0 && (
          <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <p className="text-sm text-gray-500">Total Support Given</p>
              <p className="text-2xl font-extrabold text-jamaica-gold-dark mt-1">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {events.map((event) => {
              const eventSupports = getSupportersForEvent(event.id);
              const total = getTotalReceived(event.id);
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-gray-500">
                        ...
                        {eventSupports.length > 0 && (
                          <span className="inline-flex items-center gap-1 text-jamaica-green font-medium">
                            ...
                            {eventSupports.length} supporter
                            {eventSupports.length !== 1 ? "s" : ""} &middot; $
                            {total.toFixed(2)}
                          </span>
                        )}
                      </div>
